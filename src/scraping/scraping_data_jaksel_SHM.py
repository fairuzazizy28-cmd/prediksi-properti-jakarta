...
TARGET_SAMPEL = int(POPULASI_JAKSEL_TERBARU * 0.10 * 0.8)  

# URL target 
base_url = 'https://www.rumah123.com/jual/jakarta-selatan/rumah/?certificates%5B%5D=1&maxPrice=50000000000&minPrice=500000000&transactedIncluded=1'
...
NAMA_FILE_OUTPUT = 'data_jaksel_SHM_final.csv'
# ==============================================================================

semua_data_properti = []
halaman = 1

print(f"=== STRATIFIED SAMPLING: SHM JAKARTA SELATAN ===")
...
while len(semua_data_properti) < TARGET_SAMPEL:
    ...
    try:
        ...
        if halaman % 100 == 0:
            ...
            df_temp.to_csv(f'backup_shm_jaksel_{len(semua_data_properti)}_baris.csv', index=False)

        # setting waktu delay
        jeda = random.uniform(5, 8)
        time.sleep(jeda)
        halaman += 1
